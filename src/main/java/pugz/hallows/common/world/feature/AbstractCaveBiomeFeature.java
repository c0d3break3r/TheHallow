...
public abstract class AbstractCaveBiomeFeature extends Feature<CaveBiomeFeatureConfig> {
    public AbstractCaveBiomeFeature(Codec<CaveBiomeFeatureConfig> codec) {
        super(codec);
    }
        ...
        for(int l1 = k; l1 <= k + j1; ++l1) {
            for(int i2 = i1; i2 <= i1 + j1; ++i2) {
                if (l <= reader.getHeight(Heightmap.Type.OCEAN_FLOOR_WG, l1, i2)) {
                    return this.func_207803_a(reader, generator, rand, config, d0, d1, d2, d3, d4, d5, k, l, i1, j1, k1);
                }
            }
        }
        return false;
